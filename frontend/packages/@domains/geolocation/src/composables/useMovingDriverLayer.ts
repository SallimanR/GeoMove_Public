...
import { $coords } from "maps/stores/mapsStore.ts"
import {
	MAP_CENTER_LAT,
	MAP_CENTER_LON,
	type UseMovingIconLayerOptions,
	useMovingIconLayer,
	useMovingIconLayerMaplibre,
} from "maps"
...
export function useMovingDriverLayer(
	...
	initialLat = MAP_CENTER_LAT,
	initialLon = MAP_CENTER_LON,
	popupComponent?: Component,
	noDeckGL = false
) {

	const opts: UseMovingIconLayerOptions = {
		paths: $movingDrivers,
		iconUrl: carIconUrl(),
		iconWidth: 28,
		iconHeight: 29,
	...
	const { start: startAnimation } = noDeckGL ? useMovingIconLayerMaplibre(opts) : useMovingIconLayer(opts)

	const { startPolling, stopPolling } = useMovingDriverPolling(provider, initialLat, initialLon)

	$coords.subscribe((coords) => {
		stopPolling()
		startPolling(coords.center?.lat, coords.center?.lon)
	})
